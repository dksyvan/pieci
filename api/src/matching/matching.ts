 ...
 *
 * Portage backend de `app/src/lib/matching.ts` (lui-même portage de
 * l'implémentation de référence du prototype) — voir
 * Phase2_Specs_et_Algorithmes.md section 3. Source unique du calcul de
 * `correspondances.score` / `correspondances.niveau_confiance` : ne pas
 * dupliquer cette logique en SQL.
 *
 * Note de passage à l'échelle (cf. specs section 3.6) : en amont, le
 * `MatchingService` doit pré-filtrer les candidats via `ST_DWithin` (et
 * idéalement `type_piece`) avant d'appeler `scoreMatch`/`trouverMatches`
 * sur le sous-ensemble obtenu.
 */
...
/** Score minimal à partir duquel un rapprochement est retenu. */
 ...
 *
 * Référence prototype : seuil de rétention `score >= 0.55`.
 */
...
// ---------------------------------------------------------------------------
// Niveau de confiance (persisté en base)
// ---------------------------------------------------------------------------

/**
 * Niveau de confiance persisté pour un score donné (`correspondances.niveau_confiance`).
 * >= 0.80 forte · 0.65-0.80 probable · 0.55-0.65 a_verifier.
 */
export function niveauConfiance(score: number): NiveauConfiance {
  if (score >= SEUIL_FORTE) return NiveauConfiance.FORTE;
  if (score >= SEUIL_PROBABLE) return NiveauConfiance.PROBABLE;
  return NiveauConfiance.A_VERIFIER;
}

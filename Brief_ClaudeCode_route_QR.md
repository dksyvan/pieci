...
**Stocke les adresses IP.** C'est nécessaire pour ce besoin :on ne veut pas juste compter les scans par support..

Si une donnée géographique est souhaitée plus tard, utilise l'IP brute ou le **pays** fourni par Cloudflare (`request.cf.country`), il faut que les données permettent d'identifier les personnes
